...
Sentence S semantically denotes a partition of subsets of a subset of variables. For example, High sentences from experiment 1 below, assuming X is a four-way categorical variable, can be $$\llbracket S_{high}\rrbracket = \{\{x_1\},\{x_2\},\{x_3\},\{x_4\}\},$$ referred to as *finest* below, or $$\llbracket S_{high} \rrbracket = \{\{x_1\},\{x_2,x_3,x_4\}\},$$ referred to as *one_vs_rest* below.
...
Table below is an overview of the measures applied to Experiment 1.
...
<!-- ```{r}
fit_human <- brm(
  participant_response ~ claim_kind * x_val + (1 | pid),
  data = KLexp1,
  family = cumulative(),
  file = "models/fit_human",
...
loo_comp <- loo_compare(list(
  KL_finest      = loo(fit_KL_finest),
  KL_one_vs_rest = loo(fit_KL_one_vs_rest),
  MF             = loo(fit_MF)
))
loo_comp
```
