...
Kinney and Lombrozo (2024) use **only *finest* representations** in their model.
...
And we can visualise predictions from the various measures:
```{r}
KL_finest_plot <- KLexp1 |>
  ...
  labs(
    ...
    y = "CS",
    fill = "Causal claim kind",
    title = "Predictions from KL_finest_CS"
  ) +
  theme_minimal()
...
KL_one_vs_rest_plot <- KLexp1 |>
  ...
  labs(
    ...
    y = "CS",
    fill = "Causal claim kind",
    title = "Predictions from KL_one_vs_rest_CS"
  ) +
  theme_minimal()
...
MF_plot <- KLexp1 |>
  ...
  labs(
    ...
    fill = "Causal claim kind",
    title = paste0(
      "Predictions from MF_CS  (weights: one_vs_rest ",
...
```{r fit-setup}
factor_response <- function(data) {
  data |>
    mutate(
      participant_response = factor(
        participant_response,
        levels = -3:3,
        ordered = TRUE
      )
...
```{r}
# fit_KL_finest <- brm(
#   participant_response ~ claim_kind * KL_finest_CS + (1 | pid),
#   data = factor_response(KLexp1),
#   family = cumulative()
# )
# fit_KL_one_vs_rest <- NULL
# fit_MF <- NULL
```
